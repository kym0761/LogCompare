...
public class OllamaOptions : ILlmOptions
{
    /// <summary>
    /// Ollama 서비스 또는 OpenAI 호환 로컬 API의 엔드포인트 주소
    /// </summary>
    public string Endpoint { get; set; } = "http://localhost:11434/v1";

    /// <summary>
    /// Ollama에 등록하여 구동 중인 기본 로컬 LLM 모델 식별자 (예: gemma4:e4b, llama3 등)
    /// </summary>
    public string ModelId { get; set; } = "gemma4:e4b";
    ...
    /// <summary>
    /// Ollama 서버 측 모델이 필수로 확보해야 할 컨텍스트 윈도우 크기 (128k = 131072)
    /// 주의: OpenAI API 호환성 상 C#에서 직접 주입할 수 없으므로, Ollama 서버 측 Modelfile(num_ctx) 설정 정보의 문서화용으로 쓰입니다.
    /// </summary>
    public int NumCtx { get; set; } = 131072;
